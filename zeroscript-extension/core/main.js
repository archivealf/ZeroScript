...
(() => {
  ...
  // ════════════════════════════════════════════════════════════════════════
  const ui = (() => {
    let root, bar, dot, brandEl, stateEl, actionBtn, stopBtn, switchBtn, supportBtn, discordEl, menuEl, unstableEl, retryBtn;
    ...
    function build() {
      ...
      root.innerHTML = `
        <div id="zs-bar">
          ...
          <span id="zs-brand">ZeroScript <span class="zs-free">v${esc(EXT_VERSION)}</span></span>
          ...
          <button id="zs-retry-composer" title="Retry composer" style="margin-left:6px">⟳ Retry</button>
        </div>
        <div id="zs-menu" hidden></div>
        ${P.unstableWarning ? `<button id="zs-unstable" aria-label="Provider may be unstable" hidden title="${esc(P.unstableWarning)}">⚠ unstable</button>` : ""}
      `;
      ...
      retryBtn = root.querySelector("#zs-retry-composer");
      bar.classList.add(`zs-prov-${P.id}`); // lets CSS tune per-site (e.g. font)
      if (retryBtn) {
        retryBtn.addEventListener('click', async () => {
          diag('ui.retryComposer.clicked', {});
          try { ui.banner('info', 'Retrying composer…'); } catch {}
          try {
            const res = P.ensureComposerReady ? await P.ensureComposerReady(3) : { ready: false };
            if (res && res.ready) {
              ui.banner('info', 'Composer ready');
              diag('ui.retryComposer.ok', res);
            } else {
              ui.banner('warn', 'Composer still unavailable');
              diag('ui.retryComposer.fail', res);
            }
          } catch (e) {
            diag('ui.retryComposer.err', { e: String(e) });
            try { ui.banner('error', 'Retry failed'); } catch {}
          }
        });
      }
    ...
    function buildMenu() {
      ...
      for (const s of AI_SITES) {
        ...
        const label = `<span class="zs-site-name"><span>${esc(s.name)}</span><span class="zs-site-host">${esc(hostOf(s.url))}</span></span>`;
        sites += current
          ? `<div class="zs-site-opt zs-site-here">${label}<span class="zs-site-badge">active</span></div>`
          : `<button class="zs-site-opt" data-u="${esc(s.url)}">${label}<span class="zs-site-go">&rarr;</span></button>`;
      }
      let passes = "";
      for (const p of ROBUX_PASSES) {
        passes += `<button class="zs-tip-opt zs-tip-rbx" data-u="${esc(passUrl(p.id))}"><span class="zs-rbx-cur">R$</span>${esc(String(p.robux))}</button>`;
      }
      ...
      menuEl.innerHTML =
        `<div class="zs-menu-head"><span class="zs-menu-logo">ZeroScript</span><span class="zs-menu-tag">v${esc(EXT_VERSION)}</span></div>
         ...
         <section class="zs-menu-sec">
           ...
           <button class="zs-tip-opt zs-tip-star" data-u="${esc(GITHUB_URL)}"><span>Star on GitHub</span><span class="zs-tip-sub">free, helps a lot</span></button>
           ${WORKINK_URL ? `<button class="zs-tip-opt zs-tip-ad" data-u="${esc(WORKINK_URL)}"><span>Watch an ad to support</span><span class="zs-tip-sub">free, takes a minute</span></button>` : ""}
         </section>
         <section class="zs-menu-sec">
           <div class="zs-sec-label"><span>Support with Robux / Ko-fi</span></div>
           <button class="zs-tip-opt zs-tip-kofi" data-u="${esc(KOFI_URL)}"><span>Tip on Ko-fi</span><span class="zs-tip-sub">any amount</span></button>
      ...
      function setMcpBusy(on, label) {
        ...
        if (on) {
        const labelText = label || "Restarting bridge…";
        mcpStatus.textContent = "";
        const row = document.createElement("span");
        row.className = "zs-mcp-spin-row";
        const spin = document.createElement("span");
        spin.className = "zs-mcp-spin";
        row.appendChild(spin);
        row.appendChild(document.createTextNode(labelText));
        mcpStatus.appendChild(row);
      } else {
        mcpStatus.textContent = "";
      }
      }
    ...
    function buildSetup() {
      ...
      const videoBtn = VIDEO_URL
        ? `<a id="zs-setup-video" href="${esc(VIDEO_URL)}" target="_blank" rel="noopener">▶ Watch tutorial</a>`
        : "";
      setupCard.innerHTML =
        ...
        `<div class="zs-setup-copy-row">` +
          `<input type="text" id="zs-setup-link" readonly>` +
      ...
      const linkInput = setupCard.querySelector("#zs-setup-link");
      if (linkInput) linkInput.value = esc(GITHUB_URL);
